...
export async function POST(req: NextRequest) {
  ...
  // 5. Verify home has kinde_org_code (required for invite flow)
  const [home] = await sql`
      SELECT kinde_org_code, name FROM homes WHERE id = ${homeId} LIMIT 1
    `
  if (!home?.kinde_org_code) {
    return NextResponse.json(
      { error: 'This home is not fully configured. Please contact support.' },
      { status: 400 }
    )
  }

  // 6. Find the manager's staff record (for invited_by FK)
  const managerStaff = await sql`
    SELECT id FROM staff
    WHERE kinde_user_id = ${actorId}
      AND home_id = ${homeId}
      AND deleted_at IS NULL
    LIMIT 1
  `
  ...
  // 7. Generate token and insert invite
